...
$(function(){
    $(window).scroll(function() {
        if( $(this).scrollTop() >= $("#scroll-off").height()) {            
            $(".pageTop_button").fadeIn(500);
        }else {
            
            $(".pageTop_button").fadeOut(20);
        }
    });

    //20230314フローティングバナーがフッターまでいったら消す
    var footer = $('.base-footer').innerHeight();
    ...
    $(window).scroll(function () {
        ...
        if(point > docHeight-dispHeight-footer){
          $('.flaotingBanner').fadeOut(500);
        }else{
          $('.flaotingBanner').fadeIn(500);
        }
    });
});
